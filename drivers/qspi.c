...
static uint32_t qspi_256bytes_write(QSPI_T *QSPI, uint32_t addr, uint32_t count, uint8_t *src);
static uint32_t qspi_writechar(QSPI_T *QSPI, uint32_t addr, uint8_t ch);

uint8_t qspi_set_write_status(QSPI_T *QSPI, uint8_t cmd, uint8_t data)
{
...
uint32_t qspi_page_program(QSPI_T *QSPI, uint32_t addr, uint32_t count, uint8_t *src, uint8_t spiLane)
{
    SPI_CTRL_T ctrl;
    uint8_t status;
    ...
    if (count < 256) {        		       
        for (int i = 0; i < count; i++) {
            qspi_writechar(QSPI, addr + i, src[i]);
        }        
    } else {
        qspi_256bytes_write(QSPI, addr, count, src);
    }
...
uint32_t qspi_256bytes_write(QSPI_T *QSPI, uint32_t addr, uint32_t count, uint8_t *src)
{
    SPI_CTRL_T ctrl;
    uint8_t status;
    uint32_t *data;
...
static uint32_t qspi_config_mode_pio(QSPI_T *QSPI, uint8_t mode, uint8_t spiLane, uint16_t qspiClkDivisorValue)
{    
	SPI_CONFIG_T config;
